...
const moduleConfig = (moduleSystem, file, minimized, emitDeclaration) => {
  try {
    ...
    if (!existsSync(input)) {
      console.log(
        `The source entry point was set as "${input}" but this was not found!`
      );
      console.log();
      exit(1);
    }
    ...
    const tsconfigOverride = {
      compilerOptions: {
        ...(emitDeclaration
          ? { declaration: emitDeclaration, declarationDir }
          : {}),
        outDir,
        module: "esnext",
      },
    ...
    console.warn(
      `- 👎 the build failed building Rollup configuration: ${e.message}`
    );
    console.log(`\n${e.stack}\n`);
    process.exit(1);
  }
...
async function buildModule(m, min, emitDeclaration) {
  try {
    ...
    console.log(
      `- 🚀 bundling saved to the "${file}" file [ ${
        Math.floor(output.size / 100) / 10
      } kb ].\n`
    );
  } catch (e) {
...
(async () => {
  ...
  if (moduleSystems.includes("iife") || moduleSystems.includes("umd")) {
    console.log(
      `- The IIFE and UMD modules will link to global scope:\n\t${Object.keys(
        ...
        .map(
          (g) => `- "${g}" module found in global scope as "${globals[g]}"\n\t`
        )
        .join("")}`
    );
  }
  ...
  for (const m of moduleSystems) {
    const emitDeclaration =
      moduleSystems.length === 1 || getModuleShortname(m) === "es";
